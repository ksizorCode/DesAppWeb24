...
$isHome=true; //es la página de inicio
...
GROUP BY 
    ...
    $h1='Resultados de búsqueda para <span class="destacado">'.$busqueda.'</span>'; 

} 
else {
    //Definir consulta
    $sql="SELECT r.nombre AS nombre_restaurante,r.id, r.nombre, r.foto, r.direccion, r.telefono, r.lat, r.lon, r.web, r.email, r.descripcion, r.extracto, r.slug, r.icono,
    GROUP_CONCAT(c.nombre) AS categorias,
    GROUP_CONCAT(c.icono) AS iconos
    ...
    GROUP BY r.id LIMIT 32";


    $placeholder="Buscar restaurantes... ";
    $titulo='Kuchillo de Palo';
    $h1='Buscar Restaurantes…';
...
<form action="" class="formBuscador">
    ...
    <input type="text" name="b" id="inptBuscar" list="listarestaurantes"  onkeyup="filtrar()" placeholder="<?php echo $placeholder; ?>" title="Buscar restaurantes, lugares, tipos de comida...">
...
if (isset($datos) && !empty($datos)) {
    ...
    foreach($datos as $dato){
        echo '<li style="background-image:url(assets/img/'.$dato['foto'].')">';
            ...
            img($dato['foto'],$alt,'r_foto');

            echo '<div>';
            echo '<h2 class="r_nombre">'.$dato['nombre'].'</h2>';
            echo '<span class="r_extracto" >'.$dato['extracto'].'</span>';
            echo '<span class="r_telefono" >'.$dato['telefono'].'</span>'; 
            echo '<span class="r_direccion">'.$dato['direccion'].'</span>';
            echo '<span class="r_web" >'.$dato['web'].'</span>';
            echo '<span class="r_email">'.$dato['email'].'</span>';

            //echo '<a href="info.php?id='.$dato['id'].'" class="btn">Ver más</a>';
            echo '<a href="'.$dato['slug'].'" class="btn r_vermas">Ver más</a>';

            echo '<span class="r_categorias">';
...
function filtrar() {
  ...
  for (i = 0; i < li.length; i++) {
    elements = li[i].querySelectorAll("h2, .extracto, .telefono, .direccion, .web, .email, a, div, span");
    existe = false;
    for (j = 0; j < elements.length; j++) {
      //txtValue = elements[j].textContent || elements[j].innerText;
		       txtValue = normalizar(elements[j].textContent || elements[j].innerText).toUpperCase();
